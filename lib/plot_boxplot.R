...
for (fp in pathFiles) {
  ...
  pref <- fp[[2]]
  throughput <- list(log10(df$r_throughput), "lightskyblue", "number_of_reads")
  throughput[4] <- paste(pref, "number of reads (log10)", sep = ", ")

  mdlength <- list(log10(df$median_length), "tomato", "median_length")
  mdlength[4] <- paste(pref, "median read length (log10)", sep = ", ")

  mxlength <- list(log10(df$max_length), "firebrick", "max_length")
  mxlength[4] <- paste(pref, "max read length (log10)", sep = ", ")

  phred <- list(df$normalized_phred_score, "gold", "avg_error_rate")
  phred[4] <- paste(pref, "average error rate (normalized phred score)", sep = ", ")
  
  depth <- list(df$depth, "mediumblue", "depth of coverage")
  depth[4] <- paste(pref, "depth of coverage", sep = ", ")
